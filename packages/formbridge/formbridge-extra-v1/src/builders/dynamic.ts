 ...
 * formbridge schema ready to pass to `useForm()`.
 *
 * @example
...
/**
 * `parseJsonSchema()` — convert a standard JSON Schema (draft-07) into a formbridge schema.
 *
 * @example
 * const jsonSchema = {
 *   title: 'User',
...
export function parseJsonSchema(
  ...
  for (const [name, prop] of Object.entries(jsonSchema.properties)) {
    ...
    // Map JSON Schema type + format to formbridge field type
