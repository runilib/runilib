...
export async function validateField(
  ...
  // ── min ───────────────────────────────────────────────────────────────────
  if (d._min !== undefined) {
    if (typeof v === 'string' && v.length < d._min) return d._minMsg ?? `Min ${d._min} characters.`;
    if (typeof v === 'number' && v < d._min)         return d._minMsg ?? `Min value is ${d._min}.`;
  }

  // ── max ───────────────────────────────────────────────────────────────────
  if (d._max !== undefined) {
    if (typeof v === 'string' && v.length > d._max) return d._maxMsg ?? `Max ${d._max} characters.`;
    if (typeof v === 'number' && v > d._max)         return d._maxMsg ?? `Max value is ${d._max}.`;
  }

  // ── pattern ───────────────────────────────────────────────────────────────
  if (d._pattern && typeof v === 'string') {
    if (!d._pattern.test(v)) return d._patternMsg ?? 'Invalid format.';
  }
