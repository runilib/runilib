 ...
 * and get back a fully working formbridge form.
 *
 * @example
 * // From an async API
...
export function useDynamicForm(
  ...
  // TODO should modify useFormBridge to return null or schema to avoid conditionally call of hooks
  const innetF = useFormBridge(form as FormSchema, options)

  // We use a stable hook call — when form is null, we skip
  const innerForm = form
    ? // eslint-disable-next-line react-hooks/rules-of-hooks
      useFormBridge(form as FormSchema, options)
    : null;
