...
export const asyncStorageAdapter: StorageAdapter = {
  ...
  async setItem(key, value) {
    try { (await requireAsyncStorage()).setItem(key, value); } catch { /* silent */ }
  },
  async removeItem(key) {
    try { (await requireAsyncStorage()).removeItem(key); } catch { /* silent */ }
  },
};

function requireAsyncStorage(): any {
  try {
    ...
    try {
      ...
      throw new Error(
        '[formbridge] persist with storage:"async" requires ' +
