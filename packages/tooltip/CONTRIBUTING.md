# Contributing to @runilib/tooltip

Thank you for your interest in contributing to `@runilib/tooltip`.
...
- [Contributing to @runilib/tooltip](#contributing-to-runilibtooltip)
  ...
  - [Local development](#local-development)
    ...
    - [Work on the tooltip package](#work-on-the-tooltip-package)
...
The official source code for `@runilib/tooltip` lives in the **main Runilib monorepo**.

### Main repositories

- **Main repository (source of truth):** `runilib`
- **This repository:** read-only mirror of `@runilib/tooltip`
...
If you want to contribute code to `@runilib/tooltip`, you must contribute through the main Runilib monorepo.
...
3. Create a branch
4. Work inside `packages/tooltip`
...
Anyone who needs to contribute to `@runilib/tooltip` should normally be added to the **main Runilib monorepo**, not to this mirror repository.
...
```bash
yarn workspace @runilib/tooltip dev
...
```bash
feat/tooltip-add-anchor-offset-support
fix/tooltip-web-mask-click-outside
refactor/tooltip-native-overlay-positioning
docs/tooltip-update-readme
```
...
- the change is made in packages/tooltip
- the package builds successfully
- typecheck passes
- lint passes
- tests pass
...
```bash
feat(tooltip): add ensureVisible before step measurement
fix(tooltip): correct arrow alignment on mobile
refactor(tooltip): split web and native overlay logic
docs(tooltip): add contributor guide
test(tooltip): cover spotlight transition logic
```
