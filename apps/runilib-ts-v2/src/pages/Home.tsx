    ...
    <Wrap>
      ...
      <HeroSection>
        ...
        <HeroRight>
          <OrbitalScene>
            ...
            <OrbCenter><FloatBox><LogoIcon size={84} animated /></FloatBox></OrbCenter>
            <LibOrb pos={{ top: '3%', left: '50%', transform: 'translateX(-50%)' }} color="blue">formbridge</LibOrb>
            <LibOrb pos={{ bottom: '12%', left: '6%' }} color="amber">stepwise</LibOrb>
            <LibOrb pos={{ bottom: '12%', right: '6%' }} color="teal">tooltip</LibOrb>
          </OrbitalScene>
        </HeroRight>
      </HeroSection>
        ...
        <StripInner>
          ...
          <StripCmds>
            {(['npm install formbridge stepwise @runilib/tooltip',
               'yarn add formbridge stepwise @runilib/tooltip'] as const).map((cmd, i) => (
              <StripCmd key={i}>
                <Prompt>{i === 0 ? 'npm' : 'yarn'}</Prompt>
                <CmdText>{i === 0 ? 'install' : 'add'} formbridge stepwise @runilib/tooltip</CmdText>
              </StripCmd>
            ))}
          </StripCmds>
        </StripInner>
        ...
        <LibsGrid>
          {LIBRARIES.map(lib => (
            <LibCard key={lib.id} to={`/libraries/${lib.id}`}>
              ...
              <LibCardTags>
                {lib.tags.map(tag => <LibTag key={tag}>{tag}</LibTag>)}
              </LibCardTags>
              <LibCardFeats>
                {lib.highlights.slice(0, 4).map(h => (
                  <LibFeat key={h}><Arrow2>→</Arrow2>{h}</LibFeat>
                ))}
              </LibCardFeats>
      ...
      <CTASection>
        ...
        <CTAContent>
          ...
          <CTABtns>
            <PrimaryBtn to="/docs">{t.cta.primary} <Arrow>→</Arrow></PrimaryBtn>
