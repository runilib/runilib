            ...
            <LibRow key={lib.id}>
              <RowLeft>
                ...
                <RowInfo>
                  ...
                  <RowTags>{lib.tags.map(tag => <Tag key={tag}>{tag}</Tag>)}</RowTags>
                </RowInfo>
              </RowLeft>
              ...
              <RowRight>
                <HighlightList>
                  {lib.highlights.slice(0, 5).map(h => (
                    <HighlightItem key={h}><Arrow>→</Arrow>{h}</HighlightItem>
                  ))}
                ...
                <RowActions>
                  ...
                  <GhBtn href={lib.githubUrl} target="_blank" rel="noopener">{t.libs.github}</GhBtn>
                </RowActions>
              </RowRight>
            </LibRow>
          ))}
...
const SearchBox = styled.div`
  ...
  &:focus-within { border-color: ${({ theme }) => theme.teal}; }
`
...
const RowIcon = styled.div<{ $color: LibColor }>`
  font-size: 28px;
...
const RowVer = styled.div`
  ...
  border-radius: 3px;
`
const RowStatus = styled.div<{ $status: string }>`
  font-family: 'DM Mono', monospace;
  font-size: 9.5px;
  padding: 2px 7px;
  border-radius: 10px;
...
const HighlightItem = styled.li`
  ...
  color: ${({ theme }) => theme.textSecondary};
`
const HighlightMore = styled.div`
  font-family: 'DM Mono', monospace;
  font-size: 11px;
  color: ${({ theme }) => theme.textMuted};
  font-style: italic;
...
const RoadCard = styled.div`
  ...
  opacity: 0.65;
`
const RoadIcon = styled.div<{ $color: LibColor }>`
  font-size: 22px;
...
const RoadName = styled.div`
  ...
  color: ${({ theme }) => theme.textPrimary};
`
const RoadDesc = styled.div`
  font-family: 'Sora', sans-serif;
  font-size: 12px;
  color: ${({ theme }) => theme.textSecondary};
  line-height: 1.5;
`
const PlannedTag = styled.div`
  font-family: 'DM Mono', monospace;
  font-size: 9.5px;
  color: ${({ theme }) => theme.textMuted};
  border: 1px dashed ${({ theme }) => theme.border};
