    ...
    <Wrap>
      <Hero>
        <HeroInner>
          ...
          <Sub>
            Each library has its own complete documentation with installation guide,
            API reference, and real-world examples.
          </Sub>
        </HeroInner>
      </Hero>
      ...
      <Body>
        <CardsGrid>
          {LIBRARIES.map((lib, i) => (
            <DocCard key={lib.id} to={`/libraries/${lib.id}`} style={{ animationDelay: `${i * 80}ms` }}>
              ...
              <CardFeatures>
                {lib.highlights.slice(0, 3).map(h => (
                  <CardFeature key={h}><Arrow>→</Arrow>{h}</CardFeature>
                ))}
              </CardFeatures>
        ...
        <QuickRef>
          ...
          <QuickGrid>
            <QuickCard>
              ...
              <QuickDesc>All libraries share the same locale system. Call setLocale() once.</QuickDesc>
            ...
            <QuickCard>
              ...
              <QuickDesc>Every schema is fully typed. Types are inferred — no manual annotations needed.</QuickDesc>
...
`
const CardIcon = styled.div<{ $color: LibColor }>`
  font-size: 30px;
