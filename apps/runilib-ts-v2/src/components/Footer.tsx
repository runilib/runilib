  ...
  const cols = [
    ...
    },
    {
      ...
      links: [
        ...
        { label: t.contributing.nav, to: '/contributing' },
          ...
          <Brand>
            ...
            <Badges>
              {(['MIT License', 'TypeScript', 'React 18+'] as const).map((b, i) => (
                <Badge key={b} $color={(['teal','blue','amber'] as const)[i]}>{b}</Badge>
              ))}
            </Badges>
          </Brand>
          ...
          <Cols>
            {cols.map(col => (
              <Col key={col.title}>
                ...
                {col.links.map(l => (
                  l.href
                    ? <ColExt key={l.label} href={l.href} target="_blank" rel="noopener">{l.label}</ColExt>
                    : <ColInt key={l.label} to={l.to!}>{l.label}</ColInt>
                ))}
              </Col>
            ))}
          </Cols>
