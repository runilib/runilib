...
const KEYWORDS = new Set([
  ...
  'const','let','var','function','class','new',
  'return','if','else','for','while','do','switch','case','break','continue',
  'try','catch','finally','throw',
  'async','await',
  'type','interface','extends','implements','abstract',
  'public','private','protected','readonly','static',
  'enum','namespace','declare','module',
  'typeof','instanceof','in','of','keyof',
  'void','never','any','unknown','this','super',
])
const BOOLEANS = new Set(['true','false','null','undefined','NaN','Infinity'])
const BUILTINS = new Set([
  'console','Math','Object','Array','String','Number','Boolean','Date',
  'Promise','Map','Set','Error','JSON','window','document',
  'React','useState','useEffect','useRef','useMemo','useCallback',
  'useContext','useReducer','forwardRef','createContext',
])
...
function tokenize(code: string, _lang: string): Token[] {
  ...
  while (i < code.length) {
    ...
    if (/[a-zA-Z_$]/.test(code[i])) {
      ...
      const prevChar = tokens.length > 0 ? tokens[tokens.length - 1].value.trimEnd().slice(-1) : ''

      if (KEYWORDS.has(val)) {
        tokens.push({ type: 'keyword', value: val })
      } else if (BOOLEANS.has(val)) {
        tokens.push({ type: 'boolean', value: val })
      } else if (BUILTINS.has(val) || (val[0] === val[0].toUpperCase() && prevChar !== '.')) {
        ...
        tokens.push({ type: 'funcName', value: val })
      } else if (prevChar === '<' || (prevChar === '/' && tokens[tokens.length - 2]?.value === '<')) {
    ...
    if ('=><>|&+-*/%^~'.includes(code[i])) {
      let val = code[i++]
      if ((val === '=' || val === '!' || val === '<' || val === '>') && code[i] === '=') val += code[i++]
...
function renderLine(line: string, lang: string, lineNum: number): React.ReactNode {
  if (lang === 'bash') {
    const [cmd, ...rest] = line.split(' ')
    if (cmd === '#') return <span style={{ color: C.comment }}>{line}</span>
    const isCmd = ['npm','yarn','pnpm','npx','git','cd','echo','export','mkdir'].includes(cmd)
    if (isCmd) return (
      <>
  ...
  return tokens.map((tok, i) => (
    <span key={i} style={{ color: C[tok.type] }}>{tok.value}</span>
  ))
}
