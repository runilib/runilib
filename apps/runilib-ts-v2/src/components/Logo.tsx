...
interface LogoIconProps { size?: number; animated?: boolean }
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 72 72" fill="none">
      ...
      <ellipse cx="36" cy="36" rx="33" ry="33"
        stroke="#00e5c8" strokeWidth="1.2" strokeDasharray="5 4" opacity="0.3" />
      {animated ? (
        <>
          <AnimRing1 cx="36" cy="36" rx="25" ry="10"
            stroke="#00e5c8" strokeWidth="1" opacity="0.18" />
          <AnimRing2 cx="36" cy="36" rx="25" ry="10"
            stroke="#4f8ef7" strokeWidth="1" opacity="0.18"
            style={{ transform: 'rotate(70deg)', transformOrigin: '36px 36px' }} />
        </>
      ) : (
        <>
          <ellipse cx="36" cy="36" rx="25" ry="10"
            stroke="#00e5c8" strokeWidth="1" opacity="0.18"
      ...
      <text x="36" y="43" textAnchor="middle"
        fontFamily="Sora, sans-serif" fontWeight="800" fontSize="18" fill="#f4f6fa">
        R
      </text>
    </svg>
  )
...
interface LogoFullProps { size?: number; hideTagline?: boolean }

export function LogoFull({ size = 30, hideTagline = false }: LogoFullProps) {
  return (
    <LogoLink to="/">
      <FloatIcon><LogoIcon size={size} /></FloatIcon>
      <LogoText>
        <Wordmark>
