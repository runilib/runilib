...
If you only need to restyle the default renderer, prefer [\`globalConfigs\`](/docs/globalconfigs) or the per-field \`ui\` prop first. If you need a **brand new field type** that no builder covers, jump to [\`field.custom()\`](/docs/field-custom) instead.`,
    },
    {
...
- Prefer [\`globalConfigs\`](/docs/globalconfigs) or per-field \`ui\` over \`fieldController\` when the change is purely visual — styling, spacing, label alignment — and the underlying markup is fine.`,
    },
  ],
};
