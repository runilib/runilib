...
export const overviewSection: LibraryDoc['sections'][number] = {
  id: 'fb-overview',
  title: 'Overview',
  content: `REACT FORMBRIDGE is a schema-driven form builder and runtime for React and React Native.

Define your form once with a declarative schema. Get type-safe fields, validation, conditional logic, persistence, and multi-step flows — on web and native — without wiring a single input by hand.`,
  subsections: [
    {
...
We built React FormBridge because we believe form logic should be written once and run everywhere your React code runs. The schema you write for a checkout flow on web should power the exact same flow in your native app, with the same validation, the same conditional fields, and the same type safety.`,
    },
    {
...
- **Cross-platform by design.** The same \`useFormBridge()\` hook powers web and React Native. Platform-specific UI concerns are handled through typed \`ui\` overrides, not separate codebases.
