...
export const BASE_BUILDER_METHODS = [
  ...
  '',
  buildMethodsTable([
    ['`defaultValue`', '`(value) => this`', 'Overrides the initial value'],
    ['`required`', '`(message?) => this`', 'Marks the field as required'],
    ['`optional`', '`() => this`', 'Removes the required flag'],
    ['`label`', '`(text) => this`', 'Overrides the display label'],
    ['`placeholder`', '`(text?) => this`', 'Sets placeholder copy'],
    ['`hint`', '`(text) => this`', 'Sets helper text'],
    ['`disabled`', '`(value = true) => this`', 'Toggles disabled state'],
    ['`hidden`', '`(value = true) => this`', 'Toggles hidden state'],
    ['`debounce`', '`(ms) => this`', 'Changes validation/change debounce'],
    ['`validate`', '`(fn) => this`', 'Adds a sync or async validator'],
    ['`transform`', '`(fn) => this`', 'Normalizes the value before validation / submit'],
    [
      '`render`',
      '`(fn) => this`',
      'Replaces the generated UI while keeping the same form runtime',
    ],
  ]),
  '',
  ...
  buildMethodsTable([
    [
      ...
      'Shows the field when another field or predicate matches',
    ],
    [
      '`visibleAndRequiredWhen`',
      '`(fieldOrFn, value?) => this`',
      'Combines visibility and required logic in one call',
    ],
    [
      '`visibleWhenNot`',
      '`(field, value) => this`',
      'Shows the field when another field does not match',
    ],
    [
      '`visibleWhenTruthy`',
      '`(field) => this`',
      'Shows the field when another field is truthy',
    ],
    [
      '`visibleWhenFalsy`',
      '`(field) => this`',
      'Shows the field when another field is falsy',
    ],
    ['`visibleWhenAny`', '`(pairs) => this`', 'OR-combines multiple visibility checks'],
    [
      '`requiredWhen`',
      '`(fieldOrFn, value?) => this`',
      'Makes the field conditionally required',
    ],
    ['`requiredWhenAny`', '`(pairs) => this`', 'OR-combines multiple required checks'],
    [
      '`disabledWhen`',
      '`(fieldOrFn, value?) => this`',
      'Disables the field conditionally',
    ],
    ['`resetOnHide`', '`() => this`', 'Resets to the default value when hidden'],
    ['`keepOnHide`', '`() => this`', 'Keeps the current value when hidden'],
    ['`clearOnHide`', '`() => this`', 'Clears the value when hidden'],
  ]),
].join('\n');
...
export const STRING_BUILDER_METHODS = [
  ...
  '',
  buildMethodsTable([
    ['`min`', '`(length, message?) => this`', 'Sets the minimum accepted length'],
    ['`max`', '`(length, message?) => this`', 'Sets the maximum accepted length'],
    [
      '`pattern`',
    ...
    [
      ...
      'Alias for passing multiple regex alternatives',
    ],
    [
      '`format`',
      '`(regex, message?) => this`',
      'Overrides the low-level built-in format regex used by preset builders such as email, tel, or url',
    ],
    ['`trim`', '`() => this`', 'Trims the value before validation / submit'],
    ['`lowercase`', '`() => this`', 'Lowercases the value before storing'],
    ['`uppercase`', '`() => this`', 'Uppercases the value before storing'],
    [
      '`nonEmpty`',
      '`(message?) => this`',
    ...
    [
      ...
      'Requires equality with another field value (supports `ref()` paths)',
    ],
    [
      '`sameAs`',
      '`(fieldName, message?) => this`',
      'Alias of `matches()` with more explicit semantics',
    ],
  ]),
].join('\n');
...
export const FORM_STATE_SURFACE = [
  ...
  '',
  buildMethodsTable([
    ['`values`', '`SchemaValues`', 'Current schema-shaped values object'],
    ['`errors`', '`Record<string, string>`', 'Current field error bag'],
    ['`touched`', '`Record<string, boolean>`', 'Per-field touched flags'],
    ['`dirty`', '`Record<string, boolean>`', 'Per-field dirty flags'],
    [
      '`status`',
      "`'idle' \\| 'validating' \\| 'submitting' \\| 'success' \\| 'error'`",
      'Aggregate runtime status',
    ],
    ['`isValid`', '`boolean`', 'No current validation errors'],
    [
      '`isDirty`',
      '`boolean`',
      'At least one field differs from its initial/default value',
    ],
    ['`isSubmitting`', '`boolean`', 'Submit handler currently running'],
    ['`isSubmitSuccess`', '`boolean`', 'Last submit completed successfully'],
    ['`isSubmitError`', '`boolean`', 'Last submit ended in error'],
    ['`submitCount`', '`number`', 'Number of submit attempts'],
    [
      '`submitError`',
      '`string \\| null`',
      'Mapped submit error string from `onSubmitError`',
    ],
  ]),
].join('\n');
