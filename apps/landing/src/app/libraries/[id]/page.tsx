...
export default function LibraryDetail() {
  ...
  return (
    <Wrap>
      <Sidebar>
        <SidebarTop>
          <BackLink href="/libraries">← {t.libraryPage.back.replace('← ', '')}</BackLink>
          <LibHeader>
  ...
  return parseRichTextBlocks(content).map((block, index) => {
    ...
    if (lines.length > 0 && lines.every((line) => line.startsWith('- ')))
      return (
        <BulletList key={`list-${index.toString()}`}>
          {lines.map((line, i) => (
            <li key={`item-${i.toString()}`}>{renderInlineText(line.slice(2))}</li>
          ))}
        </BulletList>
      );
    ...
    return (
      <p key={`para-${index.toString()}`}>
        {lines.map((line, i) => (
          <React.Fragment key={`line-${i.toString()}`}>
            {i > 0 ? <br /> : null}
...
const SectionTitle = styled.h1`font-family: 'Sora', sans-serif; font-size: clamp(22px, 3.5vw, 34px); font-weight: 800; color: ${({ theme }) => theme.textPrimary}; margin-bottom: 18px; letter-spacing: -0.3px;`;
...
const SubTitle = styled.h2`font-family: 'Sora', sans-serif; font-size: 18px; font-weight: 700; color: ${({ theme }) => theme.textPrimary}; margin-bottom: 12px;`;
