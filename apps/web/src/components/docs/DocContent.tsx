      ...
      <Tip
        c={
          <>
            ...
            which native element you wrap with <IC c="<CopilotStep>" />.
          </>
        }
      />
      <H2 c="Features" />
...
function QuickStart() {
  return (
    <>
      ...
      <CodeBlock
        filename="App.tsx"
        code={`import { CopilotProvider } from 'tooltip';\n\nexport default function App() {\n  return (\n    <CopilotProvider animationType="bounce">\n      <MyScreen />\n    </CopilotProvider>\n  );\n}`}
      />
      <H3 c="2. Register steps" />
      <CodeBlock
        filename="MyScreen.tsx"
        code={`import { CopilotStep } from 'tooltip';\n\n<CopilotStep name="search" order={1} title="Search" text="Find anything here.">\n  <input placeholder="Search…" />\n</CopilotStep>`}
      />
      <H3 c="3. Start the tour" />
      <CodeBlock
        filename="StartButton.tsx"
        code={`import { useCopilot } from 'tooltip';\n\nfunction StartButton() {\n  const { start } = useCopilot();\n  return <button onClick={() => start()}>Start Tour</button>;\n}`}
      />
      <Tip
        c={
          <>
            💡 <IC c="useCopilot" /> must be used inside a <IC c="<CopilotProvider>" />.
          </>
        }
      />
    </>
...
function Provider() {
  const rows: [string, string, string, string][] = [
    ...
    ["theme", "CopilotTheme", "—", "Color overrides for the tooltip"],
    ...
    ["maskClickable", "boolean", "false", "Close on backdrop click"],
    ["labels", "CopilotLabels", "—", "Override next/prev/finish/close labels"],
  ...
  return (
    <>
      ...
      <Tip
        c={
          <>
            📱 On React Native, <IC c="<CopilotStep>" /> wraps its child in a{" "}
  ...
  return (
    <>
      ...
      <Lead
        c={
          <>
            Main hook to control the tour. Must be used inside a <IC c="<CopilotProvider>" />.
          </>
        }
      />
      <CodeBlock
...
function Animations() {
  ...
  return (
    <>
      ...
      <CodeBlock code={`<CopilotProvider animationType="bounce">`} />
    </>
  );
}
